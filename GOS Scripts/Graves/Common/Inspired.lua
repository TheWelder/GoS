...
function ObjectLoopEvent(object, myHero)
	...
	if teamMates[GetNetworkID(object)] and GetObjectType(Object) == GetObjectType(myHero) and GetTeam(Object) == GetTeam(myHero) then
        teamMates[GetNetworkID(object)] = object
    end
...
function GetTeamAround(pos, range)
    local c = 0
    if pos == nil then return 0 end
    for k,v in pairs(GetTeamHeroes()) do 
        if v and ValidTarget(v) and GetDistanceSqr(pos,GetOrigin(v)) < range*range then
            c = c + 1
        end
    end
