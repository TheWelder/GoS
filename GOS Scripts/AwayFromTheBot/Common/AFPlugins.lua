...
local myMaxHp = GetMaxHP(myHero)
	if "Soraka" == GetObjectName(myHero) then
		if GetDistance(closestEnemyTurret(myHeroPos),myHero) > 1500 then
			...
			if myHp/myMaxHp > 0.3 and GetCurrentHP(closestAllyHero(myHeroPos))/GetMaxHP(closestAllyHero(myHeroPos)) < 0.6 and IsInDistance(closestAllyHero(myHeroPos),500) then
				if CanUseSpell(myHero,_W) == READY then
					CastTargetSpell(closestAllyHero(myHeroPos),_W)
				end
			end
